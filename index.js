...
const uninstallWin = async (settings, job, fontpath) => {
    ...
    while (retries < NX_FONT_REMOVAL_RETRIES) {
        ...
        settings.logger.log(`[action-fonts] Removing font ${fontdest} (${retries}/${NX_FONT_REMOVAL_RETRIES})`);
...
module.exports = async (job, settings, params, type) => {
    ...
    if (type == "prerender") {
        ...
        for (const asset of job.assets) {
            settings.logger.log(`[action-fonts] Processing asset ${asset.dest}...`);
            if (asset.type !== "static") {
                continue;
            }

            if (!asset.dest.match(/\.(ttf|otf)$/)) {
                continue;
            }

            if (!asset.name) {
                throw new Error(`Asset ${asset.dest} has to be named using the "name" property that would contain the font name as it is used to be then used in the After Effets project.`);
            }
        ...
        for (const asset of job.assets) {
            ...
            if (!asset.dest.match(/\.(ttf|otf|woff|woff2)$/)) {
                continue;
            }
